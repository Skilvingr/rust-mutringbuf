...
use mutringbuf::{ConcurrentHeapRB, Iterable, LocalHeapRB};
...
fn test_push_work_pop_slice() {
    let (mut prod, mut work, mut cons) = LocalHeapRB::new_heap(BUFFER_SIZE + 1).split_mut(0);
    ...
    if let Some(((h, t), _)) = work.get_workable_slice_exact(BUFFER_SIZE) {
